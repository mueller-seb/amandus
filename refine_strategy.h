...
namespace RefineStrategy
{
  ...
  {
  public:
    MarkMaximum(dealii::Triangulation<dim> &tria,
    ...
    void operator()(const dealii::Vector<double> &indicator)
    {
      dealii::GridRefinement::refine_and_coarsen_fixed_number(*(this->tria),
                                                              indicator,
                                                              this->refine_threshold,this->coarsen_threshold);
    }
  ...
  {
  public:
    MarkBulk(dealii::Triangulation<dim> &tria,
             double refine_threshold,
             double coarsen_threshold = 0.) :
      AmandusRefineStrategy<dim>(tria),
      refine_threshold(refine_threshold),
    ...
    void operator()(const dealii::Vector<double> &indicator)
    {
      dealii::GridRefinement::refine_and_coarsen_fixed_fraction(*(this->tria),
                                                                indicator,
                                                                this->refine_threshold,this->coarsen_threshold);
    }
  ...
  {
  public:
    MarkOptimize(dealii::Triangulation<dim> &tria,

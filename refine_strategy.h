 ...
 * @ingroup Postprocessing
 **/
template <int dim> class AmandusRefineStrategy {
public:
  AmandusRefineStrategy(dealii::Triangulation<dim> &tria) : tria(&tria) {}
  virtual void operator()(const dealii::Vector<double> &indicator) = 0;

protected:
  dealii::SmartPointer<dealii::Triangulation<dim>, AmandusRefineStrategy<dim>>
      tria;
};
 ...
 * @ingroup Postprocessing
 **/
template <int dim> class MarkUniform : public AmandusRefineStrategy<dim> {
public:
...
/**
 * @brief mark fixed number, i.e. maximum criterium
 *
 * @ingroup Postprocessing
 **/
template <int dim> class MarkMaximum : public AmandusRefineStrategy<dim> {
public:
  MarkMaximum(dealii::Triangulation<dim> &tria, double refine_threshold,
              double coarsen_threshold = 0.)
      : AmandusRefineStrategy<dim>(tria), refine_threshold(refine_threshold),
        coarsen_threshold(coarsen_threshold) {}

  void operator()(const dealii::Vector<double> &indicator) {
    dealii::GridRefinement::refine_and_coarsen_fixed_number(
        *(this->tria), indicator, this->refine_threshold,
...
/**
 * @brief mark fixed fraction, i.e. bulk criterion
 *
 * @ingroup Postprocessing
 **/
template <int dim> class MarkBulk : public AmandusRefineStrategy<dim> {
public:
  MarkBulk(dealii::Triangulation<dim> &tria, double refine_threshold,
           double coarsen_threshold = 0.)
      : AmandusRefineStrategy<dim>(tria), refine_threshold(refine_threshold),
        coarsen_threshold(coarsen_threshold) {}
 ...
 * @ingroup Postprocessing
 **/
template <int dim> class MarkOptimize : public AmandusRefineStrategy<dim> {
public:
  MarkOptimize(dealii::Triangulation<dim> &tria, const unsigned int order = 2)
      : AmandusRefineStrategy<dim>(tria), order(order) {}

  void operator()(const dealii::Vector<double> &indicator) {
    dealii::GridRefinement::refine_and_coarsen_fixed_fraction(
        *(this->tria), indicator, this->order);
  }

      ...
      ConstrainedNewton<VECTOR>::operator() (AnyData &out, const AnyData &in)
      {
        ...
        if (this->control.last_check() != SolverControl::success)
          AssertThrow(false, SolverControl::NoConvergence(
                  this->control.last_step(), this->control.last_value()));
        // otherwise exit as normal
      }


    template<class VECTOR>
      bool ConstrainedNewton<VECTOR>::constraint_violation(const VECTOR& u) {
        double gamma = u[0];
        double sigma = u[1];
        double rho = u[2];
        ...
        if(!(std::max(0.0, numbers::PI - numbers::PI * gamma) < 
             -2 * sigma * gamma && 
             -2 * sigma * gamma <
             std::min(numbers::PI, 2 * numbers::PI - numbers::PI * gamma)))
        { 
          return true;
        }
        return false;

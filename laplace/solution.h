 ...
 * @ingroup integrators
 */
template <int dim> class SolutionRHS : public AmandusIntegrator<dim> {
public:
 ...
 * @ingroup integrators
 */
template <int dim> class SolutionResidual : public AmandusIntegrator<dim> {
public:
...
template <int dim> class SolutionError : public AmandusIntegrator<dim> {
public:
...
template <int dim> class SolutionEstimate : public AmandusIntegrator<dim> {
public:
...
template <int dim>
SolutionRHS<dim>::SolutionRHS(Function<dim> &solution) : solution(&solution) {
  this->use_boundary = true;
  this->use_face = false;
}

template <int dim>
void SolutionRHS<dim>::cell(DoFInfo<dim> &dinfo,
...
template <int dim>
void SolutionRHS<dim>::boundary(DoFInfo<dim> &dinfo,
                                IntegrationInfo<dim> &info) const {
  ...
  for (unsigned k = 0; k < fe.n_quadrature_points; ++k)
    for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
      local_vector(i) +=
          (penalty * fe.shape_value(i, k) * boundary_values[k] -
           (fe.normal_vector(k) * fe.shape_grad(i, k)) * boundary_values[k]) *
          fe.JxW(k);
}

template <int dim>
void SolutionRHS<dim>::face(DoFInfo<dim> &, DoFInfo<dim> &,
                            IntegrationInfo<dim> &,
...
template <int dim>
void SolutionResidual<dim>::cell(DoFInfo<dim> &dinfo,
                                 IntegrationInfo<dim> &info) const {
...
template <int dim>
void SolutionResidual<dim>::boundary(DoFInfo<dim> &dinfo,
                                     IntegrationInfo<dim> &info) const {
  ...
  Laplace::nitsche_residual(
      dinfo.vector(0).block(0), info.fe_values(0), info.values[0][0],
      info.gradients[0][0], boundary_values,
      Laplace::compute_penalty(dinfo, dinfo, deg, deg), factor);
}

template <int dim>
void SolutionResidual<dim>::face(DoFInfo<dim> &dinfo1, DoFInfo<dim> &dinfo2,
                                 IntegrationInfo<dim> &info1,
                                 IntegrationInfo<dim> &info2) const {
  const unsigned int deg = info1.fe_values(0).get_fe().tensor_degree();
  const double factor = (this->timestep == 0.) ? 1. : this->timestep;
  Laplace::ip_residual(
      dinfo1.vector(0).block(0), dinfo2.vector(0).block(0), info1.fe_values(0),
      info2.fe_values(0), info1.values[0][0], info1.gradients[0][0],
...
template <int dim>
void SolutionError<dim>::cell(DoFInfo<dim> &dinfo,
                              IntegrationInfo<dim> &info) const {
...
template <int dim>
void SolutionError<dim>::boundary(DoFInfo<dim> &,
                                  IntegrationInfo<dim> &) const {}

template <int dim>
void SolutionError<dim>::face(DoFInfo<dim> &, DoFInfo<dim> &,
                              IntegrationInfo<dim> &,
                              IntegrationInfo<dim> &) const {}
...
template <int dim>
void SolutionEstimate<dim>::cell(DoFInfo<dim> &dinfo,
                                 IntegrationInfo<dim> &info) const {
  const FEValuesBase<dim> &fe = info.fe_values();
...
template <int dim>
void SolutionEstimate<dim>::boundary(DoFInfo<dim> &dinfo,
                                     IntegrationInfo<dim> &info) const {
  const FEValuesBase<dim> &fe = info.fe_values();
...
template <int dim>
void SolutionEstimate<dim>::face(DoFInfo<dim> &dinfo1, DoFInfo<dim> &dinfo2,
                                 IntegrationInfo<dim> &info1,
                                 IntegrationInfo<dim> &info2) const {
  ...
  const double penalty =
      2. * Laplace::compute_penalty(dinfo1, dinfo2, deg1, deg2);

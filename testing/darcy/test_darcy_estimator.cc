...
BOOST_AUTO_TEST_CASE(postprocessor_test)
{
  ...
  FESystem<dim> solution_fe(FE_RaviartThomas<dim>(0), 1, FE_DGQ<dim>(0), 1);
  ...
  VectorTools::project(
    solution_dofh, solution_constraints, solution_quadrature, test_function, solution);
  ...
  Postprocessor<dim> postprocessor(pp_dofh, solution_dofh, weight);
  ...
  FEValues<dim> pp_fev(pp_fe, pp_quadrature, update_values | update_quadrature_points);
...
double
InterpolationTestFunction::value(const Point<2>& p, const unsigned int /*component*/) const
{
  if (p(0) > 0.0 && p(1) < 0.0)
  {
    return 0.0;
...
BOOST_AUTO_TEST_CASE(interpolator_test)
{
  ...
  VectorTools::project(input_dofh, input_constraints, input_quadrature, test_function, input);

  Interpolator<dim> interpolator(input_dofh, input_fe.tensor_degree() + 2, weight);
  Vector<double> interpolation;
  interpolator.init_vector(interpolation);
  interpolator.interpolate(interpolation, input);

  Functions::FEFieldFunction<dim> interpolation_function(interpolator.get_dofh(), interpolation);

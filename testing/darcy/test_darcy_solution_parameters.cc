...
BOOST_AUTO_TEST_CASE(test_parameters)
{
  ...
  Darcy::Checkerboard::SolutionParameters solution_parameters(coefficient_parameters);

  BOOST_CHECK_EQUAL(solution_parameters.parameters.size(), control_solution_parameters.size());
  for (unsigned int i = 0; i < solution_parameters.parameters.size(); ++i)
  {
    BOOST_CHECK_CLOSE(solution_parameters.parameters[i], control_solution_parameters[i], TOL);
  }
}

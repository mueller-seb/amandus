...
Residual<dim>::cell(DoFInfo<dim>& dinfo, IntegrationInfo<dim>& info) const
{
  ...
  {
    minus_d_potential[q] = (point[1][q] * (1.0 - point[1][q] * point[1][q])) / diffusion;
  }

  L2::L2(dinfo.vector(0).block(0), info.fe_values(0), point[0]);
  L2::L2(dinfo.vector(0).block(0), info.fe_values(0), minus_d_potential);
  Laplace::cell_residual(dinfo.vector(0).block(0), info.fe_values(0), Dpoint[1], -1.0 * diffusion);

  Laplace::cell_residual(dinfo.vector(0).block(1), info.fe_values(1), Dpoint[0]);

  dealii::LocalIntegrators::Advection::cell_residual(
    dinfo.vector(0).block(1), info.fe_values(1), Dpoint[1], direction);
}
}

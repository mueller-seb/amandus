                                ...
                                std::vector<Vector<double>>& values) const
{
  ...
  for (unsigned int k = 0; k < points.size(); ++k)
  {
    ...
    values[k](0) =
      (values[k](1) * values[k](1) - 1) * values[k](1) + eps_square * 2.0 * values[k](1);
  }
}
...
template <int dim>
void
CrossFunction<dim>::vector_value_list(const std::vector<Point<dim>>& points,
                                      std::vector<Vector<double>>& values) const
{
  AssertDimension(points.size(), values.size());
...
template <int dim>
void
CrossFunction<dim>::value_list(const std::vector<Point<dim>>& points, std::vector<double>& values,
                               const unsigned int component) const
{
  AssertDimension(points.size(), values.size());
...
public:
  ...
  value(const Point<2>& p, const unsigned int component = 0) const
  {
    if (component == 1)
    {
      if (std::abs(p(1)) < 0.5 && (std::abs(p(0)) < 0.05 || ((p(0) > 0 && p(0) < -2.0 * p(1)) ||
                                                             (p(0) < 0 && p(0) > -2.0 * p(1)))))
      {
        return 1.0;
      }
...
template <int dim>
Function<dim>*
advectionselector(int i, double strength = 0.0)
{
  switch (i)
  {
    case 0:

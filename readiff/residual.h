...
Residual<dim>::cell(DoFInfo<dim>& dinfo, IntegrationInfo<dim>& info) const
{
  ...
  Laplace::cell_residual(
    dinfo.vector(0).block(0), info.fe_values(0), info.gradients[0][0], p.alpha1);
  Laplace::cell_residual(
    dinfo.vector(0).block(1), info.fe_values(0), info.gradients[0][1], p.alpha2);
}
